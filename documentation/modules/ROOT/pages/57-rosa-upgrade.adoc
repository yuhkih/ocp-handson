...
1台ずつワーカーノードの新規追加と削除が自動的に実行されます。

更新の際に、削除対象のワーカーノード上でPodが実行している場合、Podの停止と再作成が実行されますので、
冗長性を考慮してPodのレプリカ数を複数個設定しておくことを推奨します。
...
なお、ワーカーノードの更新にかかる所要時間は、ワーカーノード上で実行されているPodの停止に必要な時間に依存します。
既存ワーカーノードの削除の際に、一定時間以上停止できなかったPodについては、強制的に削除されます。
Pod削除前の猶予時間について、OCMの当該クラスターの「Settings」タブから指定できます。
デフォルトの猶予時間は「1時間」となっており、これはいくつかの選択肢から指定して変更できます。
