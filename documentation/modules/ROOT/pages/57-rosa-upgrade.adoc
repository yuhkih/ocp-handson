...
「ROSA CLIを利用したワーカーノードの更新」以外の項目は、実施できません。
...
[OCMにログイン](https://console.redhat.com/openshift/)して、
...
新しいマイナーバージョン(4.15や4.16など)がリリースされた場合は、利用者に通知され、
...
2023年12月時点では、現在の時刻から約5分後に更新されるようスケジューリングされます。
...
指定したMachinepol、または全てのMachinepoolを更新できます。
...
1台ずつワーカーノードの新規追加と削除を実行していきます。

更新の際に、削除対象のワーカーノード上でPodが実行している場合、Podの停止と再作成が実行されますので、
冗長性を考慮して、Podのレプリカ数を複数個設定しておくことを推奨します。
...
なお、ワーカーノードの更新にかかる所要時間は、ワーカーノード上で実行されているPodの停止時間に依存します。
既存ワーカーノードの削除の際に、一定時間以上停止できなかったPodについては、強制的に削除されます。
Pod削除前の猶予時間について、OCMの当該クラスターの「Settings」タブから指定できます。
デフォルトの猶予時間は1時間となっており、これはいくつかの選択肢から指定して変更できます。

NOTE: 2023年12月時点では、ROSA HCPクラスターの場合、ROSA CLIでこの猶予時間を設定できないようになっています。

image::rosa/upgrade/node-drain.png[]
...
=== ROSA CLIによるワーカーノードの更新

NOTE: 本演習をワークショップ形式で実施している場合、下記で紹介している手順を実施してください。
...
? Autorepair: Yes
I: Machine pool 'mp01' created successfully on hosted cluster 'hcp-01'
...
Machinepool更新のスケジュールを削除したい場合は、 `rosa delete upgrade` コマンドを実行します。
