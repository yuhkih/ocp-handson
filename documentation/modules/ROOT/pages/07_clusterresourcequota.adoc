...
[Note]
====
`user1` は、`kubeadmin` によって `pricelist-frontend` と `pricelist-backend` のユーザとして割り当てられているため、決められた数の *Pod* を作成することができます。`user1` は *Project* を作成していないので、`openshift.io/requester=user1` annotationはありません。あなたの環境に合わせてクォータポリシーをミックスしたり、マッチさせたりすることができることがわかります。
====
