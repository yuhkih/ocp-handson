...
== 演習の概要
このモジュールでは、ROSAクラスターのプロジェクトのアプリケーションについて、アラートを設定します。
...
プロジェクトを選択して、ROSAクラスターのWebコンソール右上にある、「+」アイコンをクリックします。
...
また、PrometehusメトリクスをROSAクラスター内部で見るために利用する「prometheus-example-app」
...
「prometheus-example-app」サービスを作成したプロジェクトを選択して、「+」アイコンをクリックしてServiceMonitorを作成します。
...
「for: 30s」では、アラート発行のための条件式が真となって、アラートが「pending(保留中)」状態から「firing(実行中)」
...
NOTE: 「通知」にあるスイッチをOFFにすると、アラートをサインレンス状態にできます。
...
[Note]
====
ROSAやOpenShiftでは、アラートを外部システムに送信できます。対応しているシステムは下記となります。
