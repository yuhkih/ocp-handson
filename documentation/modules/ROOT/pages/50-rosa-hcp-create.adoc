...
ROSAは、Red Hatによるお客様の既存AWSアカウントへのデプロイを可能にするモデルを提供します。Red Hatはセキュリティを考慮して、https://docs.aws.amazon.com/ja_jp/IAM/latest/UserGuide/id_credentials_temp.html[AWS Secrity Token Service (STS)] を使用することを、ROSA HCPクラスター作成の前提条件としています。STSを使用したROSA HCPクラスター作成の前提条件は、次のドキュメントをご参照ください。
