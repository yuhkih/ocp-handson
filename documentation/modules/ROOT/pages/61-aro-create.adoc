...
NOTE: AROクラスターのワーカーノードは最小3台からとなります。
また、利用者のワークロードに応じて、ワーカーノードのサイズ変更もできます。
利用できるワーカーノードのタイプについては、
https://learn.microsoft.com/ja-jp/azure/openshift/support-policies-v4#worker-nodes[Microsoftの公式ドキュメント]
をご参照ください。
