...
== 演習の概要
このモジュールでは、ワーカーノードの(自動的な)追加・削除を実施します。
...
インストラクターによって案内されます。以下の手順は、 `hcp-01` という名前のROSAクラスターを例とします。
また、1つのROSAクラスターを受講生で共有している場合、他の受講生と重複しないMachinepoolの名前を付けて下さい。
...
ワーカーノードに対応したAWS EC2インスタンス(デフォルトはm5.xlarge)を2台起動しているという設定を確認できます。
ここにmachinepoolを新しく作成して、コンピュートノードを1台追加します。 `rosa create machinepool` コマンドを実行します。
...
NOET: 2023年12月時点では、ROSA HCPクラスターではEC2スポットインスタンスの利用はできません。
...
NOTE: 本演習をワークショップ形式で実施している場合、OCMへのアクセス権限を持たないことを想定します。
...
Machinepoolに紐づくワーカーノードに自動付与されるラベルのうち、
`hypershift.openshift.io/nodePool=<ROSAクラスター名>-<Machinepool名>` を使います。
下記の画像の例では、 `hypershift.openshift.io/nodePool=hcp-01-mp20` というラベルが付与されています。
...
適当なプロジェクト(この例では、 `autoscale-ex20` )を作成し、
...
この他にも、管理者アカウントで、ROSAクラスターの「コンピュート」→「Nodes」の情報を見ることで、
自動的にワーカーノードが追加されている状況を確認できます。

前述のコマンドで作成したm5.xlargeインスタンスのMachinepoolを利用して、
このオートスケールのテストを実行した場合、所要時間の内訳は下記となり、大体25分ほどかかります。

* ジョブの実行開始から完了まで: 15分ほど
* ジョブの実行完了から、追加されたワーカーノード1台の自動削除が完了するまで: 10分ほど


途中でジョブの実行を中止して次の演習に移りたい場合、
当該ジョブを選択して、右上の「アクション」メニューから、
「Jobの削除」を選択して「削除」を実行することで、ジョブを削除できます。
これにより、ジョブによって起動されたPodが全て削除され、
10分ほど経つと、追加されたワーカーノードが自動的に削除されます。
