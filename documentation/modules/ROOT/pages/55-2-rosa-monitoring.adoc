...
次のYAMLファイルで置き換えることで、自動的にPVCが作成されて、メトリクスデータが保存されるようになります。

OpenShiftのConfigMapは、「管理者向け表示」→「Workloads」→「ConfigMaps」から編集します。
OpenShift CLIを用いた編集方法もありますが、これについては、
