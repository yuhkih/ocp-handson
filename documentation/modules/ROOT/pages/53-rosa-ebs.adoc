...
また、
https://docs.openshift.com/rosa/storage/persistent_storage/persistent-storage-aws.html#maximum-number-of-ebs-volumes-on-a-node_persistent-storage-aws[1台のノードに接続できるEBSのボリュームは39個まで]となります。
これはAWSのEBSの制限に起因します。

[Note]
====
Amazon EFSを永続ボリュームとして利用することもできます。
