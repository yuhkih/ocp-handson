...
[Note]
====
ここではAWSリージョンのシングルアベイラビリティーゾーン(Single-AZ)に、ROSA HCPクラスターのワーカーノードを作成することを前提として、VPCを作成しています。マルチアベイラビリティーゾーン(Multi-AZ)にワーカーノードを作成して、AZ単位で冗長性を確保したい場合は、`terraform plan` コマンド実行時に、リージョンにあるAZのIDを3つ指定します。
```
$ terraform plan -out rosa.tfplan -var region=us-east-2 \
...
Multi-AZ構成のパブリッククラスターにする場合は、`subnet-ids`オプションで、VPCのサブネットを4つまたは6つ指定します。

[Note]
====
ROSA HCPクラスターのワーカーノードの
https://docs.openshift.com/rosa/rosa_architecture/rosa_policy_service_definition/rosa-hcp-service-definition.html#rosa-sdpolicy-instance-types_rosa-hcp-service-definition[最小台数は2台、最大台数は51台]です。これは、Single-AZ, Multi-AZで共通しています。
