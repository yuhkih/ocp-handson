...
htpasswdの場合、本番環境で汎用的な認証プロバイダとしての利用をサポートしていませんので、ご注意ください。次のコマンドは、一時的な検証用途としてhtpasswdを利用するための設定例です。 `testuser1` ~ `testuser100` のパスワード情報を記載したファイル `users.htpasswd` を、ROSA HCPクラスターの認証プロバイダとして設定しています。
