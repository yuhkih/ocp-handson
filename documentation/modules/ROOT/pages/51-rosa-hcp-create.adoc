...
[Note]
====
作成したROSA HCPクラスターの情報は、OCMの画面からも確認できます。下記の画像は、SingleAZ構成(ワーカーノード2台)とMultiAZ構成(ワーカーノード2台と3台)の例です。
...
これで、ROSA HCPクラスターのコンソールにログインできるようになります。このコンソールのURLは、次のコマンドで確認できます。ユーザー名とパスワードは、`rosa create admin` コマンド実行時に表示されたもの(ユーザー名は `cluster-admin` )を使えます。
...
htpasswdの場合、本番環境で汎用的な認証プロバイダとして利用することはサポートしていませんので、ご注意ください。次のコマンドは、一時的な検証用途としてhtpasswdを利用するための設定例です。testuser1~testuser100のパスワード情報を記載したファイル `users.htpasswd` を、ROSA HCPクラスターの認証プロバイダとして設定しています。
