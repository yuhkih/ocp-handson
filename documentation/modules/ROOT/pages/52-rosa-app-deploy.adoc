...
NOTE: Red Hatの製品ドキュメントに、https://access.redhat.com/documentation/ja-jp/red_hat_build_of_node.js/20/html-single/node.js_runtime_guide/index#developing-and-deploying-a-nodejs-application_introduction-to-application-development-with-runtime[サンプルアプリケーションの作成ガイド]を記載していますので参考にしてください。
...
この例だと「nodejs-ex-git」という名前が付けられており、関連するリソースに「app=nodejs-ex-git」というラベル(上記画像の青四角部分)が付けられていることを確認できます。
...
ただし、2024年6月時点ではバグのためローカルユーザーの権限では削除できないようになっています。
そのため、作成したアプリケーションを削除したい場合、次のいずれかの方法を実行してください。
