= Web IDEを利用したサンプルアプリケーションの開発
:navtitle: rosa-dev-spaces
include::_attributes.adoc[]

== 演習の概要
このモジュールでは、Red Hat OpenShift Dev Spacesでのサンプルアプリケーションを開発します。
...
追加料金を支払う必要なく、使うことができます。

Dev Spacesは、OpenShiftのOperatorとして提供されており、
Operatorからインストールすることで、簡単に使えるようになっています。


[Note]
====
NOTE: 本演習をワークショップ形式で実施している場合、Dev Spacesは既にインストールして設定されています。

Dev Spacesをインストールする場合、OpenShiftの管理者アカウントで、OperatorHubから
`Red Hat OpenShift Dev Spaces` を最初にインストールします。Operatorインストール時の
...
デフォルトで「YAMLビュー」選択されていますが、一旦「フォームビュー」を選択して、
再度「YAMLビュー」を選択することで、インスタンス作成時のパラメーターが表示されるようになります。
ここで編集しておくと、便利なパラメーターを記載します。

* `maxNumberOfRunningWorkspacesPerUser` : OpenShiftの各ユーザーアカウントで、同時に実行できる
Workspaceの数。デフォルトは「1」となっていて、同時実行数は1です。「-1」とすると、無制限になります。
* `secondsOfInactivityBeforeIdling` : Workspaceを操作しなかったときに、Workspaceが停止するまでの秒数であり、
...
「Java 11 with Quarkus」をクリックして、JDK, Maven, 
...
このIDEは、Visual Studio Code (VS Code)のように利用することができます。
...
Mavenの実行ファイルやソースコード一式が入っていますので、ここから、次のMavenコマンドを実行して、
サンプルアプリを実行してみます。

NOTE: 本演習では、Quarkusの解説は含みません。
このサンプルアプリケーションがどういう仕組みになっているか、について興味がありましたら、
...
このコマンド実行によって、必要なパッケージがダウンロードされ、jarファイルが作成されて、
Quarkusのアプリケーションが実行されます。正常に実行が完了すると、
...
下記のように、 `... more options> `の所で、半角「q」を入力することで、
Quarkusアプリケーションを終了できますので、停止後に再度Quarkusアプリケーションを、
Mavenコマンドで実行してください。
...
下記のような画面が表示されれば、Quarkusアプリケーションが正常に実行されています。
...
左サイドに表示されている `getting-started` ディレクトリから辿っていって、
...
実際に、これを確認してみてください。下記のように、URLの末尾に `/hello` を加えて、
アクセスすることで、`hello` というメッセージだけの画面が表示されます。
...
に変更して、Control-sキーなどで、 `GreetingResource.java` ファイルを上書き保存します。
...
アプリケーションが実行したままとなり、今回の例だと、
他のWorkspaceでのアプリケーション実行時に、8081ポートが使えなくなりますので、ご注意ください。

[Note]
====
Workspaceの停止や削除は、左サイドメニューの「Workspaces」から、対象のWorkspaceの
「Stop Workspace」や「Delete Workspace」から実行できます。Workspace自身も、
`<ユーザーアカウント名>-devspaces` プロジェクトにあるPodとして実行されており、
Workspaceの停止や削除を実行すると、それに対応したPodも削除されます。
...
「Delete Workspace」からWorkspaceを削除すると、利用者が加えた変更は完全に消去されます。
====


=== Gitリポジトリのソースコードからのアプリケーションデプロイ

Web IDEを使って開発したコードををGitリポジトリに置いておくことで、
OpenShiftのS2I機能を利用した、アプリケーションデプロイを実行できます。

本演習では各受講者用のリポジトリは提供していないため、
...
あとは、しばらく待つと、アプリケーションのビルドとデプロイが自動的に完了しますので、
