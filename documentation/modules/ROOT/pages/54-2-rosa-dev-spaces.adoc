...
下記のように、 `\[h] for more options> `の所で、半角「q」を入力することで、
Quarkusアプリケーションを終了できますので、終了後に再度Quarkusアプリケーションを、
`mvn` コマンドで実行してください。
...
そのため、
https://aws.amazon.com/jp/codewhisperer/[Amazon CodeWhisperer]が使える状態だと、
https://marketplace.visualstudio.com/items?itemName=AmazonWebServices.aws-toolkit-vscode&ssr=false#version-history[AWS Toolkit]
拡張機能のファイルを手動でダウンロードして、Dev Spacesの各Workspaceにインポートすることで、
Amazon CodeWhispererによるコード補完機能などが使えるようになります。
